...
export class TernaryEntryTypeToggleComponent implements OnInit, OnDestroy {
  ...
  onChange = (value: number) => {
    const update$ = (t: ReportCardEntryType) =>
      this.service.update(this.entry.labwork.course, t.id, {
        ...t,
        bool: this.fromNumber(value),
      })

    mapUndefined(this.currentType(), (t) =>
      this.subs.push(subscribe(update$(t), identity)),
    )
  }

  ngOnInit(): void {
    this.state = mapUndefined(this.currentType(), this.fromReportCardEntryType)
  }
