...
export class EntryTypeBonusFieldComponent implements OnInit, OnDestroy {
  ...
  private createFormControl = () => {
    const updateChanges = (entry: Readonly<ReportCardEntryType>, v: number) => {
      const update$ = this.service.update(this.entry.labwork.course, entry.id, {
        ...entry,
        int: v,
      })
      const updateUI = (u: ReportCardEntryType) =>
        (this.entry.entryTypes = this.entry.entryTypes.map((x) =>
          x.id === u.id ? u : x,
        ))
    ...
    mapUndefined(this.maybeBonus(), (entry) => {
      ...
      const s = subscribe(
        this.fc.valueChanges.pipe(distinctUntilChanged()),
        (v) => {
          if (isValidNumber(v)) {
