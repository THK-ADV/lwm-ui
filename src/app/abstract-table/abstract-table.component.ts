...
export class AbstractTableComponent<Model> implements OnInit, OnDestroy {
  ...
  @Input() set data$($: Observable<Model[]>) {
    // TODO: this is the fix for the comment below. apply everywhere
  ...
  private setupDataSource = () => {
    ...
    mapUndefined(
      ...
      (p) => (this.dataSource.filterPredicate = p),
    )
  }
  ...
  onEdit = (model: Model) => this.editEmitter.emit(model)
