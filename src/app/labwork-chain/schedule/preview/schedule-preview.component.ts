...
} from "./schedule-preview-view-model"
...
export class SchedulePreviewComponent implements OnInit, OnDestroy {
  ...
  canDeleteAndCommit = (): ActionType[] =>
    this.hasPermission
      ? [
          { type: "delete", label: undefined },
          { type: "upload", label: undefined },
        ]
      : []
  ...
  private commit = () => {
    const dialogRef = ConfirmDialogComponent.instance(this.dialog, {
      title: "Staffelplan erstellen",
      body: { kind: "text", value: "Möchten Sie den Staffelplan erstellen?" },
    })
