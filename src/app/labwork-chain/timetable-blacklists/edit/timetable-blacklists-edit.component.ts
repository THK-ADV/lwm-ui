...
export class TimetableBlacklistsEditComponent {
  ...
  onCreate = () => {
    ...
    const payload: FormPayload<BlacklistProtocol> = {
      ...
      data: localBlacklistCreationInputData(
        `${this.labwork.label}, ${this.labwork.semester.abbreviation}`,
      ),
    ...
    this.updateDataSource$(
      openDialogFromPayload(
        ...
        addBlacklistToTimetable$(
          ...
          this.timetable,
        ), // TODO this should be handled by the backend (see delete)
      ),
    )
  }
  ...
  onDelete = (blacklist: Blacklist) => {
    const deleteRef = DeleteDialogComponent.instance(this.dialog, {
      label: fullBlacklistLabel(blacklist),
      id: blacklist.id,
    })
