...
export const removeBlacklistFromTimetable$ =
  ...
  ): ((id: string) => Observable<TimetableAtom>) =>
  (blacklistId) =>
    service.removeBlacklist(timetable.labwork.course, timetable.id, blacklistId)
...
export const addBlacklistToTimetable$ = (
  ...
  return (b) =>
    blacklistService
      ...
      .pipe(
        switchMap((bl) =>
          addBlacklistToTimetable0(timetableService, timetable, bl),
        ),
      )
}
...
const addBlacklistToTimetable0 = (
  ...
  updateTimetable$(timetableService, timetable, (u) => {
    const copy = { ...u }
...
export const updateBlacklistInTimetable$ =
  (
    ...
    timetable: Readonly<TimetableAtom>,
    id: string,
  ): ((blacklist: BlacklistProtocol) => Observable<TimetableAtom>) =>
  (blacklist) =>
    blacklistService.update(blacklist, id).pipe(
      map((updated) => {
        const copy = { ...timetable }
        copy.localBlacklist = copy.localBlacklist.map((b) =>
