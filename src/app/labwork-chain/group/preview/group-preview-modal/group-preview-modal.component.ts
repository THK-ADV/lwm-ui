...
import {
  GroupStrategy,
  SchedulePreviewConfig,
} from "../group-preview-view-model"
import {
  mapUndefined,
  parseUnsafeBoolean,
  parseUnsafeNumber,
...
export class GroupPreviewModalComponent {
  ...
  private readonly minMaxOption = {
    value: "min-max",
    viewValue: "Gruppengröße",
  }
  ...
  submit = () => {
    ...
    this.dialogRef.close(
      mapUndefined(strategy, (s) => ({
        strategy: s,
        considerSemesterIndex: this.considerSemesterIndexValue(),
      })),
    )
  }

  hasError = (fcName: string) =>
    !this.formGroup.controls[fcName].untouched &&
    this.formGroup.controls[fcName].hasError(fcName)

  getErrorMsg = (fcName: string) =>
    this.formGroup.controls[fcName].getError(fcName)

  considerSemesterIndexValue = () =>
    parseUnsafeBoolean(this.formGroup.controls.considerSemesterIndex.value)

  private foldSelectedValue = <T>(
    count: (count: number) => T,
    minMax: (min: number, max: number) => T,
    nil: () => T,
  ): T => {
      ...
      return this.foldSelectedValue<ValidationErrors | null>(
        ...
        (min, max) => {
          ...
          if (min > 0 && min < max) {
            ...
            const err = {
              max: "Minimum muss größer als 0 und kleiner als Maximum sein",
            }
