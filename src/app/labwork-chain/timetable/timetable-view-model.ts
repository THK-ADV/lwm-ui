...
export const updateTimetable$ = (
  ...
  return service.update(
    copy.labwork.id,
    copy.id,
    toTimetableProtocol(update(copy)),
  )
}
...
export const updateTime = (
  ...
  return (entry) => {
    const index =
      entry.dayIndex === dayIndex(start) ? entry.dayIndex : dayIndex(start)
    return {
      ...entry,
      start: Time.fromDate(start),
      end: Time.fromDate(end),
...
export const updateStartDate = (
  date: Date,
): ((t: Readonly<TimetableAtom>) => Readonly<TimetableAtom>) => {
