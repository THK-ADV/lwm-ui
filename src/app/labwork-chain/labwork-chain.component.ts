...
export class LabworkChainComponent implements OnInit, OnDestroy {
  ...
  private jumpToGroups = () =>
    (this.stepper.selectedIndex = Step.groups.valueOf())
  ...
  private fetchChainData = () => {
    const s1 = fetchLabwork(this.route, this.labworkService, (labwork) => {
      ...
      this.subscribeAndPush(
        fetchAssignmentEntries(this.assignmentPlanService, labwork),
        (xs) => (this.assignmentEntries = xs),
      )
      this.subscribeAndPush(
        fetchOrCreateTimetable(
        ...
        (tt) => (this.timetable = tt),
      )
      this.subscribeAndPush(
        fetchApplicationCount(this.labworkApplicationService, labwork),
        (a) => (this.applications = a),
      )
      this.subscribeAndPush(
        fetchScheduleEntries(this.scheduleEntryService, labwork),
        (s) => (this.scheduleEntries = s),
      )
      this.subscribeAndPush(
        fetchReportCardEntryCount(this.reportCardService, labwork),
        (n) => (this.reportCards = n),
      )
    })
  ...
  isStepCompleted = (step: Step): boolean => {
    switch (step) {
      ...
      case Step.closing:
        return (
          this.isStepCompleted(Step.schedule) && this.hasReportCardEntries()
        )
    }
  }

  hasPermission = (): boolean =>
    hasCourseManagerPermission(this.route, this.labwork.course.id)
  ...
  isStepLocked = (step: Step): boolean => {
    ...
    switch (step) {
      ...
      case Step.blacklists:
        return (
          this.hasSchedulePreview() ||
          this.hasScheduleEntries() ||
          this.hasReportCardEntries()
        )
      case Step.groups:
