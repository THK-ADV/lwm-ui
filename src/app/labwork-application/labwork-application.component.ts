...
export class LabworkApplicationComponent implements OnInit, OnDestroy {
  ...
  tableContent: (
    model: Readonly<LabworkApplicationAtom>,
    attr: string,
  ) => string
  ...
  ngOnInit(): void {
    this.subs.push(
      subscribe(fetchLabwork$(this.route, this.labworkService), (lab) => {
        this.labwork = lab
        this.headerTitle += ` für ${lab.label}`
        const isCM = hasAnyRole(
        ...
        this.applications$ = this.labworkApplicationService.getAllByLabwork(
          lab.course.id,
          lab.id,
        )
      }),
  ...
  private create = () => {
    this.subs.push(
      subscribe(
        this.openUpdateDialog(this.empty(), (p) =>
          this.labworkApplicationService.createForOther(this.courseId(), p),
        ),
  ...
  private download = () => {
    const s = subscribe(
      this.labworkApplicationService.download(this.courseId(), this.labwork.id),
      (blob) => {
  ...
  onEdit = (app: LabworkApplicationAtom) => {
    this.subs.push(
      subscribe(
        this.openUpdateDialog(app, (p) =>
          this.labworkApplicationService.updateForOther(
            this.courseId(),
            p,
            app.id,
          ),
  ...
  private openUpdateDialog = (
    ...
    const inputData = (): FormInput[] => {
      const makeRequest = () => {
        if (isAdmin(userAuths(this.route))) {
          // Admins can apply any students to a labwork. This is an exception!
          return this.userService.getAllWithFilter({
            attribute: "status",
            value: "student",
          })
        } else {
    ...
    const payload: FormPayload<LabworkApplicationProtocol> = {
      ...
      data: inputData(),
      makeProtocol: (output) =>
        makeProtocol(output, isUniqueEntity(app) ? app : undefined),
    }
  ...
  onDelete = (app: LabworkApplicationAtom) => {
    ...
    const dialogRef = DeleteDialogComponent.instance(this.dialog, {
      label: `Anmeldung von ${formatUser(app.applicant)}`,
      id: app.id,
    })
    ...
    const s = subscribeDeleteDialog(
      dialogRef,
      (id) =>
        this.labworkApplicationService.deleteForOther(this.courseId(), id),
      updateUI,
