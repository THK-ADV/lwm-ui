...
export class ReportCardEntryTypeBatchUpdateComponent
  implements OnInit, OnDestroy
{
  ...
  constructor(
    ...
    this.assignmentEntries = assignmentEntryService.getAllWithFilter(p.course, {
      attribute: "labwork",
      value: p.labwork,
    })
    this.students = this.reportCardEntryService
      .getAllWithFilter(p.course, { attribute: "labwork", value: p.labwork })
      .pipe(
        map((xs) =>
          mapMap(
            groupBy(xs, (e) => e.student.id),
            (_, v) => v[0].student,
          ),
        ),
      )
  ...
  entryTypes = () => (this.assignmentEntryFc.value as AssignmentEntry).types
  ...
  private validateStudents = (students: User[]) => {
    ...
    const [valid, invalid] = partitionWith(gmids, (gmid) =>
      students.find((s) => s.systemId.toLowerCase() === gmid.toLowerCase()),
    )
  ...
  private go = ($: Observable<number>) =>
    this.subs.push(
      subscribe($, (number) =>
        this.alertService.reportSuccess(`${number} Einträge aktualisiert`),
      ),
    )
