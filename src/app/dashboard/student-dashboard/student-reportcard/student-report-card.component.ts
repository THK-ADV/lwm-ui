...
export class StudentReportCardComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.fetchReportCards((cards) => {
      this.fetchReschedules(cards, (res) =>
        this.updateReportCardEntryTableUI(res),
      )
  ...
  private fetchReschedules = (
    cards: ReportCardEntryAtom[],
    completion: (
      rs: [ReportCardEntryAtom, ReportCardRescheduledAtom[]][],
    ) => void,
  ) => {
    this.subs.push(
      subscribe(
        of(cards).pipe(
          switchMap((xs) =>
            xs.map((x) => zip(of(x), this.rescheduledService.all(x.id))),
          ),
          mergeAll(),
          toArray(),
        ),
  ...
  private fetchReportCards = (
    completion: (cards: ReportCardEntryAtom[]) => void,
  ) => {
    this.subs.push(
      subscribe(
        this.route.paramMap.pipe(
          switchMap((map) => {
            ...
            return labwork && student
              ? this.reportCardEntryService.fromStudent(student, labwork)
              : EMPTY
          }),
        ),
        (cards) => {
      ...
      subscribe(
        this.route.paramMap.pipe(
          switchMap((map) => {
            const labwork = map.get("lid")
            return labwork
              ? this.annotationService.getForStudent(labwork)
              : EMPTY
          }),
        ),
  ...
  private updateReportCardEntryTableUI = (
    cards: [ReportCardEntryAtom, ReportCardRescheduledAtom[]][],
  ) => {
    this.tableModel = this.makeTableModel(cards)
  }

  private updateAnnotationsUI =
    (cards: ReportCardEntryAtom[]): ((annotations: AnnotationAtom[]) => void) =>
    (annotations) => {
          ...
          (k, v) => {
            ...
            return {
              ...
              content: v
                ...
                .map(
                  (a) =>
                    `${format(a.lastModified, "dd.MM.yyyy - HH:mm")}: ${a.message} (${fullUserName(a.author)})`,
                ),
            }
          },
    ...
    return {
      dataSource: new MatTableDataSource(
        cards
          ...
          .map(([entry, reschedules]) => ({
            entry,
            reschedules,
            annotationCount: 0,
          })),
      ),
      columns: basicColumns.concat(
        distinctEntryTypeColumns(cards.flatMap(([e]) => e.entryTypes)),
      ),
    }
  }
