...
type StudentDashboardReportCardEntry = [
  ReportCardEntryAtom,
  ReportCardRescheduledAtom | undefined,
]
type StudentDashboardCalEntry =
  | StudentDashboardReportCardEntry
  | ScheduleEntryAtom
...
export class StudentDashboardCalComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.hasCalendarEntries =
      this.reportCardEntries.length > 0 || this.scheduleEntries.length > 0
    this.scheduleEntryEvents = this.makeCalendarEvents()
  }

  private makeCalendarEvents =
    (): ScheduleEntryEvent<StudentDashboardCalEntry>[] => {
      const go = ([entry, reschedules]: [
        ReportCardEntryAtom,
        ReportCardRescheduledAtom[],
      ]): ScheduleEntryEvent<StudentDashboardReportCardEntry> => {
  ...
  isReportCardEntryAtom = (
    e: StudentDashboardCalEntry,
  ): e is StudentDashboardReportCardEntry =>
    (e as StudentDashboardReportCardEntry)[0]?.assignmentIndex !== undefined

  eventTitleFor = (
    view: CalendarView,
    e: Readonly<ScheduleEntryEvent<StudentDashboardCalEntry>>,
  ) =>
    foldUndefined(
      e.extendedProps,
      (p) =>
        this.isReportCardEntryAtom(p)
  ...
  onEventClick = (event: ScheduleEntryEvent<StudentDashboardCalEntry>) => {
    ...
    this.router.navigate([routeUrl(event.extendedProps)], {
