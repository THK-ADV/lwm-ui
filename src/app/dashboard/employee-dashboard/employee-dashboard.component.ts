...
export const employeeDashboardScheduleEntryEvents = (
  scheduleEntries: ScheduleEntryAtom[],
): ScheduleEntryEvent<ScheduleEntryAtom>[] => {
...
export class EmployeeDashboardComponent implements OnInit, OnDestroy {
  ...
  cards = (xs: ScheduleEntryAtom[]): Card<CourseAtom, ScheduleEntryAtom>[] => {
    ...
    forEachMap(
      groupBy(xs, (_) => _.labwork.course.id),
      (_, value) => {
        if (nonEmpty(value)) {
  ...
  eventTitleFor = (
    view: CalendarView,
    e: Readonly<ScheduleEntryEvent<ScheduleEntryAtom>>,
  ) =>
    (e.extendedProps && scheduleEntryEventTitleLong(view, e.extendedProps)) ||
    e.title
    ...
    this.subs.push(
      subscribe(
        ...
        (dashboard) => {
          ...
          this.entries = employeeDashboardScheduleEntryEvents(
            dashboard.scheduleEntries,
          )
  ...
  private getCurrentCourses = (scheduleEntries: ScheduleEntryAtom[]) => {
    ...
    forEachMap(
      groupBy(scheduleEntries, (e) => e.labwork.course.id),
      (k, v) => {
