...
export class GroupEditComponent implements OnInit, OnDestroy {
  // TODO apply assignment index changes
  ...
  ngOnInit() {
    this.subs.push(subscribe(this.fetchGroupsAndStudents(), this.updateUI))
  }
      ...
      switchMap((params) => {
        ...
        return (
          (cid &&
            lid &&
            gid &&
            zip(this.groupService.getAllWithFilter(cid, lid), of(gid))) ||
          EMPTY
        )
      }),
    )
  ...
  private setupGroups = (data: [GroupAtom[], string]) => {
    ...
    const [selectedGroup, otherGroups] = partition(
      allGroups,
    ...
    this.dataSource.data = selectedGroup[0].members.sort((a, b) =>
      a.lastname.localeCompare(b.lastname),
    )

    this.otherGroups = otherGroups.sort((a, b) =>
  ...
  private setupStudents = (allGroups: GroupAtom[]) => {
    const setupStudentFormGroup = () => {
      ...
      this.addStudentFormGroup.addControl(
        ...
        new FormControl(
          this.addStudentForm.data.value,
          this.addStudentForm.data.validator,
        ),
      )
    ...
    this.fellowStudents$ = this.userService
      .getAllWithFilter({ attribute: "status", value: "student" })
      .pipe(
        map((students) =>
          students.filter(
            (s) => !exists(allStudentsInGroup, (x) => x.id === s.id),
          ),
        ),
      )
  ...
  userFromControl = (): User => this.userFormControl().value as User

  shouldAllowSwapping = (): boolean => this.otherGroups.length > 0
  ...
  move = (member: User, dest: GroupAtom) => {
    const movementMsg = (result: GroupMovementResult): string => {
      ...
      let msg = result.changedMembership
        ? `moved into group ${result.newMembership.group}`
        : "failed to move into dest group"
      if (cardsUpdated) {
    ...
    const result$ = this.lwmService.moveStudentToGroup(this.courseId, {
  ...
  delete = (member: User) => {
    ...
    const delete$ = () =>
      this.lwmService.removeStudentFromGroup(this.courseId, {
        labwork: this.labworkId,
        group: this.groupId,
        student: member.id,
      })
    ...
    const result$ = openDialog(
      ConfirmDialogComponent.instance(this.dialog, {
        title: `${formatUser(member)} aus der Gruppe dauerhaft entfernen?`,
      }),
  ...
  add = (member: User) => {
    ...
    const result$ = this.lwmService.insertStudentIntoGroup(this.courseId, {
      labwork: this.labworkId,
      group: this.groupId,
      student: member.id,
    })
  ...
  fastForward = (member: User) => this.evalExplicit(member, "fastForward")

  fire = (member: User) => this.evalExplicit(member, "fire")

  private evalExplicit = (member: User, kind: ExplicitEvaluationKind) => {
    const go = () =>
      this.lwmService.evaluateExplicit(this.courseId, {
        student: member.id,
        labwork: this.labworkId,
        group: this.groupId,
        kind: kind,
