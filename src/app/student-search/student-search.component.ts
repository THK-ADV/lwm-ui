...
import {
  ReportCardTableEntry,
  ReportCardTableModel,
} from "../report-card-table/report-card-table.component"
...
class DataSource {
  ...
  clear = () => (this.dataSources = {})

  private uniqueIdentifier = (labwork: LabworkContent) => labwork.labwork.id
...
export class StudentSearchComponent implements OnInit {
  ...
  private prepareForUI = (
    dashboard: StudentSearchDashboard,
  ): StudentSearchDashboard => {
    const sortCourses = () =>
      dashboard.courses.sort((a, b) => {
        const lhs = a.labworks.some((_) =>
          this.isCurrentSemester(dashboard.semester, _),
        )
        const rhs = b.labworks.some((_) =>
          this.isCurrentSemester(dashboard.semester, _),
        )
  ...
  expandIfCurrentSemester = (
    current: SemesterJSON,
    labwork: LabworkContent,
  ): boolean => {
  ...
  bindDataSource = (labwork: LabworkContent) => {
    ...
    const tableEntries: ReportCardTableEntry[] = labwork.reportCardEntries.map(
      ({ reportCardEntry, annotations, reschedules }) => {
