...
const groupByAssignmentIndex = (
  data: Readonly<ReportCardEntry[]>,
): ReportCardEntryByAssignmentIndex =>
  toTupleArray(groupBy(data, (_) => _.assignmentIndex)).sort(
    (a, b) => a[0] - b[0],
  )

const accIfEntryType = (
  entryType: EntryType,
): ((acc: number, x: ReportCardEntry) => number) => {
  return (acc, x) => {
...
const toLineData = (
  data: ReportCardEntryByAssignmentIndex,
  entryType: EntryType,
): LineData[] => {
...
export function jsonDrawer(svg, container, scaleX, scaleY) {
  ...
  function showTooltip(x, y, d) {
    const body =
      d.ya + " / " + d.total + " (" + parseFloat(d.yp).toFixed(2) + "%)"
    const head = d.label
    ...
    tt.select("#bodyValue").text(body)

    tt.select("#headlineValue").text(head)
  ...
  function drawLegend(name, itemColor) {
    container.append("p").text(name).style("color", itemColor)
  }
