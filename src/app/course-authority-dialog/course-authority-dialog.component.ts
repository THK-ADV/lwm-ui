...
export class CourseAuthorityUpdateDialogComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.columns = [
      { attr: "user", title: "Nutzer" },
      { attr: "role", title: "Rolle" },
    ]
    this.subs = []
  ...
  static instance(
    dialog: MatDialog,
    course: CourseAtom,
  ): MatDialogRef<CourseAuthorityUpdateDialogComponent> {
        ...
        (auths) => {
          ...
          if (this.dataSource.data.length !== 0) {
            this.dataSource.data.sort((lhs, rhs) =>
              lhs.user.lastname.localeCompare(rhs.user.lastname),
            )
          }
        },
      ),
  ...
  onDelete = (auth: AuthorityAtom) => {
    this.subs.push(
      subscribe(this.authorityService.delete(this.course.id, auth.id), (_) =>
        this.afterDelete(auth),
      ),
    )
  }
  ...
  employeeCount = (): number =>
    count(
      ...
      (auth) => auth.role.label !== UserRole.courseAssistant,
    )
