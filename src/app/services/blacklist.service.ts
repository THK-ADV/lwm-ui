...
export class BlacklistService
  implements AbstractCRUDService<BlacklistProtocol, Blacklist>
{
  ...
  getAll = (): Observable<Blacklist[]> =>
    this.http.getAll<BlacklistJSON>(this.path).pipe(map(convertManyBlacklists))

  download = (year: number): Observable<PartialResult<Blacklist>> =>
    this.http
      .create<Object, PartialResult<BlacklistJSON>>(this.downloadPath(year), {})
      .pipe(
        map((pr) => ({ ...pr, created: convertManyBlacklists(pr.created) })),
      )

  preview = (year: number): Observable<Blacklist[]> =>
    this.http
      .getAll<BlacklistJSON>(this.downloadPath(year))
      .pipe(map(convertManyBlacklists))

  delete = (id: string): Observable<Blacklist> =>
    this.http.delete<BlacklistJSON>(this.path, id).pipe(map(mapBlacklistJSON))

  create = (protocol: BlacklistProtocol): Observable<Blacklist> =>
    this.http
      .create<BlacklistProtocol, BlacklistJSON>(this.path, protocol)
      .pipe(map(mapBlacklistJSON))

  createFromRange = (range: BlacklistRangeProtocol): Observable<Blacklist[]> =>
    this.http
