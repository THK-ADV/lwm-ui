...
export class AssignmentEntriesService {
  ...
  private readonly path = (course: string) =>
    makePath("assignmentEntries", course)

  getAllWithFilter = (
    courseId: string,
    ...filter: AssignmentEntryFilter[]
  ): Observable<AssignmentEntry[]> =>
    this.http.getAll(this.path(courseId), applyFilter(filter, nonAtomicParams))
  ...
  delete = (courseId: string, id: string): Observable<AssignmentEntry> =>
    this.http.delete(this.path(courseId), id)
