...
interface ReportCardEntryFilter {
    attribute: 'course' | 'student' | 'labwork' | 'semester'
    value: string
}
...
export class ReportCardEntryService {
    ...
    get = (id: string, courseId: string): Observable<ReportCardEntryAtom> => this.http
        .get(makePath(this.path, courseId), id, atomicParams)
        .pipe(map(mapReportCardEntryAtomJSON))
    ...
    rescheduleCandidates = (courseId: string, semesterId: string): Observable<RescheduleCandidate[]> => this.http
    ...
    fromStudent = (student: string, labwork: string): Observable<ReportCardEntryAtom[]> => this.http
