...
export class SemesterService implements AbstractCRUDService<SemesterProtocol, Semester> {
    ...
    current = (): Observable<Semester> => this.getAll0(new HttpParams().set('select', 'current'))
        .pipe(take(1), map(s => s[0]))

    create = (protocol: SemesterProtocol): Observable<Semester> => this.http
    ...
    update = (protocol: SemesterProtocol, id: string): Observable<Semester> => this.http
        .put<SemesterProtocol, SemesterJSON>(this.path, id, protocol)
        .pipe(map(mapSemesterJSON))

    private getAll0 = (params?: HttpParams): Observable<Semester[]> => this.http
