...
export class HttpService {
  ...
  private logResp =
    <T>(action: string, url: string) =>
    (t: T) =>
      console.debug("HTTP SERVICE", action, url, t)

  getAll = <T>(url: string, params?: HttpParams): Observable<T[]> =>
    this.http
      .get<T[]>(url, { params })
      .pipe(catchError(this.handleError), tap(this.logResp("getAll", url)))

  get = <T>(url: string, id: string, params?: HttpParams): Observable<T> =>
    this.http
      .get<T>(`${url}/${id}`, { params })
      .pipe(catchError(this.handleError), tap(this.logResp("get", url)))

  get_ = <T>(url: string, params?: HttpParams): Observable<T> =>
    this.http
      .get<T>(url, { params })
      .pipe(catchError(this.handleError), tap(this.logResp("get_", url)))

  downloadXlsSheet = (url: string): Observable<Blob> => {
    const options = {
  ...
  delete = <T>(url: string, id: string): Observable<T> =>
    this.http
      .delete<T>(`${url}/${id}`)
      .pipe(catchError(this.handleError), tap(this.logResp("delete", url)))

  delete_ = (url: string): Observable<unknown> =>
    this.http
      .delete(url)
      .pipe(catchError(this.handleError), tap(this.logResp("delete", url)))
  ...
  create = <I, O>(
    ...
    this.http
      ...
      .pipe(catchError(this.handleError), tap(this.logResp("create", url)))
  ...
  put = <I, O>(
    ...
    this.http
      ...
      .pipe(catchError(this.handleError), tap(this.logResp("put", url)))

  put_ = <I, O>(url: string, body: I): Observable<O> =>
    this.http
      .put<O>(url, body)
      .pipe(catchError(this.handleError), tap(this.logResp("put_", url)))
}
