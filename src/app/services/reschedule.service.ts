...
import {
  ...
  mapReportCardRescheduledAtomJSON,
...
export class RescheduleService {
  ...
  create = (
    courseId: string,
    protocol: ReportCardRescheduledProtocol,
  ): Observable<ReportCardRescheduledAtom> =>
    this.http
      .create<
        ReportCardRescheduledProtocol,
        ReportCardRescheduledAtomJSON
      >(makePath(this.path, courseId), protocol, atomicParams)
      .pipe(map(mapReportCardRescheduledAtomJSON))

  all = (reportCardEntryId: string): Observable<ReportCardRescheduledAtom[]> =>
    this.http
      .getAll<ReportCardRescheduledAtomJSON>(
        `reportCardEntries/${reportCardEntryId}/reportCardReschedules`,
        atomicParams,
      )
