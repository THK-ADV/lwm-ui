...
export class AlertService {
  ...
  reportAlert = (alert: Alert) => this._alerts.next(alert)

  reportSuccess = (message: string) =>
    this.reportAlert({
      type: "success",
      body: { kind: "message", value: message },
    })
  ...
  reportLWMError = (error: LWMError) =>
    this.reportAlert({
      ...
      body: {
        kind: "message",
        value: `${error.message} (status: ${error.status})`,
      },
    })
  ...
  reportError = (error: Error) =>
    this.reportAlert({
      type: "danger",
      body: { kind: "message", value: error.message },
    })
}
