...
export class LabworkApplicationService {
    ...
    getAllByLabwork = (courseId: string, labworkId: string): Observable<LabworkApplicationAtom[]> =>
        this.http.getAll(makePath(this.path, courseId, labworkId), atomicParams)

    createForSelf = (protocol: LabworkApplicationProtocol): Observable<LabworkApplicationAtom> =>
        this.http.create(this.path, protocol, atomicParams)

    createForOther = (courseId: string, protocol: LabworkApplicationProtocol): Observable<LabworkApplicationAtom> =>
    ...
    deleteForOther = (courseId: string, id: string): Observable<LabworkApplicationAtom> =>
        this.http.delete(makePath(this.path, courseId), id)

    updateForSelf = (protocol: LabworkApplicationProtocol, id: string): Observable<LabworkApplicationAtom> =>
        this.http.put(this.path, id, protocol, atomicParams)

    updateForOther = (courseId: string, protocol: LabworkApplicationProtocol, id: string): Observable<LabworkApplicationAtom> =>
