...
export class ReportCardEvaluationPatternService {
    ...
    getAll = (courseId: string, labworkId: string): Observable<ReportCardEvaluationPattern[]> =>
        this.http.getAll(makePath(this.path, courseId), applyFilter([{attribute: 'labwork', value: labworkId}]))

    delete = (id: string, courseId: string): Observable<ReportCardEvaluationPattern> =>
        this.http.delete(makePath(this.path, courseId), id)

    create = (pattern: ReportCardEvaluationPatternProtocol, courseId: string): Observable<ReportCardEvaluationPattern> =>
