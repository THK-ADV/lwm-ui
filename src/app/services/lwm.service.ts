...
export class LwmService {
  ...
  ): Observable<GroupInsertionResult> =>
    this.http.put_(makePath("insertIntoGroup", courseId), request)

  removeStudentFromGroup = (
    courseId: string,
    request: GroupChangeRequest,
  ): Observable<GroupDeletionResult> =>
    this.http.put_(makePath("removeFromGroup", courseId), request)

  moveStudentToGroup = (
    courseId: string,
    request: GroupMovingRequest,
  ): Observable<GroupMovementResult> =>
    this.http.put_(makePath("moveToGroup", courseId), request)

  evaluateExplicit = (
    courseId: string,
    request: ExplicitEvaluationRequest,
  ): Observable<ExplicitEvaluationResult> =>
    this.http.put_(makePath("evaluateExplicit", courseId), request)
}
