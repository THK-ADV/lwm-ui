...
export class NavComponent implements OnInit, OnDestroy {
  ...
  private updateUI = () => {
    const [roleAuths, moduleAuths] = partition(
      this.authorities,
      (a) => a.course === undefined,
    )

    this.user = this.authorities[0].user

    this.moduleAuthorities = moduleAuths.sort((a, b) =>
      (a.course as CourseAtom).abbreviation.localeCompare(
        (b.course as CourseAtom).abbreviation,
      ),
    )
