...
export class ApplicationListComponent implements OnInit, OnDestroy {
  ...
  @Output() applicationEmitter: EventEmitter<
    [Readonly<LabworkApplicationAtom>, "add" | "delete" | "update"]
  >
  ...
  private labworkApplication = (
    labworkId: string,
  ): LabworkApplicationAtom | undefined =>
  ...
  applicationTimestamp = (labwork: LabworkAtom) =>
    mapUndefined(this.labworkApplication(labwork.id), (app) =>
      format(app.lastModified, "dd.MM.yyyy - HH:mm"),
    )
  ...
  modifyApplication = (labwork: LabworkAtom) => {
    ...
    const $ = openDialog(
      StudentCreateApplicationComponent.instance(
        this.dialog,
        labwork,
        app.applicant.id,
        app,
  ...
  revokeApplication = (labwork: LabworkAtom) => {
    ...
    const dialogData = {
      label: `Praktikumsanmeldung für ${labwork.label} vom ${this.applicationTimestamp(labwork)}`,
      id: application.id,
    }
  ...
  apply = (labwork: LabworkAtom) => {
    ...
    this.subs.push(
      subscribe($, (x) => this.applicationEmitter.emit([x, "add"])),
    )
  }
}
