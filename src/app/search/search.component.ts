...
export class SearchComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (course) {
      this.sub = subscribe(
        this.userService.allStudentsRestricted(course.id),
        (users) => {
          users = users.sort(
            (a, b) =>
          ...
          this.filteredUser = this.userControl.valueChanges.pipe(
            ...
            startWith(""),
            map((value) =>
              typeof value === "string" ? value : this.display(value),
            ),
  ...
  displayFn = (user?: User): string | undefined => user && this.display(user)

  private display = (user: User) => formatUser(user)

  private filter = (input: string): User[] => {
    input = input.toLowerCase()
    this.toHighlight = input
    return this.users.filter(
      (t) => this.display(t).toLowerCase().indexOf(input) >= 0,
    )
  }
}
