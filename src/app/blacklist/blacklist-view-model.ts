...
export const formatBlacklistTableEntry = (
...
export const globalBlacklistInputData = (
  model: Readonly<BlacklistProtocol | Blacklist>,
  isModel: boolean,
): FormInput[] => {
  return [
    {
...
export const localBlacklistInputData = (
  model: Readonly<BlacklistProtocol | Blacklist>,
  isModel: boolean,
): FormInput[] => {
    ...
    .concat([
      {
        ...
        displayTitle: "Start",
        isDisabled: false,
        data: new FormInputTime(
          isUniqueEntity(model) ? model.start : Time.startOfTheDay(),
        ),
      },
      {
        formControlName: "end",
        ...
        isDisabled: false,
        data: new FormInputTime(
          isUniqueEntity(model) ? model.end : Time.endOfTheDay(),
        ),
      },
    ])
}
...
export const createGlobalBlacklistFromOutputData = (
  existing: Blacklist | undefined,
): ((output: FormOutputData[]) => BlacklistProtocol) => {
  return (output) =>
    withCreateProtocol(output, emptyGlobalBlacklistProtocol(), (p) => {
...
export const createGlobalBlacklistRangeFromOutputData = (
...
export const createLocalBlacklistFromOutputData = (
  output: FormOutputData[],
): BlacklistProtocol => {
...
export const updateLocalBlacklistFromOutputData = (
  blacklist: Blacklist,
): ((output: FormOutputData[]) => BlacklistProtocol) => {
  return (output) =>
    withCreateProtocol(output, emptyLocalBlacklistProtocol(), (p) => {
