...
export class AnnotationComponent implements OnInit, OnDestroy {
  ...
  constructor(
    private dialogRef: MatDialogRef<
      AnnotationComponent,
      [Annotation, AnnotationDialogAction]
    >,
  ...
  ngOnInit() {
    this.subs.push(
      subscribe(
        this.service.fromReportCardEntry(
          this.courseId(),
          this.reportCardEntry().id,
        ),
        (annotations) => {
          const [own, others] = partitionWith(annotations, (a) =>
            a.author.id === this.user().id ? a : undefined,
          )
          ...
          this.otherAnnotations = others.sort((lhs, rhs) =>
            dateOrderingASC(lhs.lastModified, rhs.lastModified),
          )
        },
      ),
    )
  ...
  private user = () => this.payload[1]

  private reportCardEntry = () => this.payload[0]

  private courseId = () => this.reportCardEntry().labwork.course
  ...
  private finishDialog = (
    ...
    this.subs.push(
      subscribe(annotation$, (a) => this.dialogRef.close([a, action])),
    )
  ...
  showAnnotationData = (a: AnnotationAtom) => a.message
  ...
  onCreate = () =>
    this.formGroup.valid &&
    this.finishDialog(
      this.service.create(this.courseId(), this.makeProtocol()),
  ...
  onDelete = () =>
    this.ownAnnotation &&
    this.finishDialog(
      this.service.delete(this.courseId(), this.ownAnnotation.id),
      AnnotationDialogAction.delete,
    )

  hasOwnAnnotation = () => this.ownAnnotation !== undefined
}
